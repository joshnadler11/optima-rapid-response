...
import teamAction2Img from '@/assets/team-action-2.jpg';
            ...
            <form onSubmit={handleSubmit} className="bg-background p-8 md:p-12 h-full">
              <div className="space-y-5">
                <div>
                  ...
                  <input required value={form.name} onChange={update('name')} className="w-full h-12 min-h-[52px] px-4 border border-border rounded-md bg-background text-foreground text-base focus:outline-none focus:ring-2 focus:ring-accent transition" />
                </div>
                <div>
                  <label className="text-sm font-semibold text-foreground mb-1.5 block">Phone</label>
                  <input required type="tel" value={form.phone} onChange={update('phone')} className="w-full h-12 min-h-[52px] px-4 border border-border rounded-md bg-background text-foreground text-base focus:outline-none focus:ring-2 focus:ring-accent transition" />
                </div>
                <div>
                  <label className="text-sm font-semibold text-foreground mb-1.5 block">Email</label>
                  <input required type="email" value={form.email} onChange={update('email')} className="w-full h-12 min-h-[52px] px-4 border border-border rounded-md bg-background text-foreground text-base focus:outline-none focus:ring-2 focus:ring-accent transition" />
                </div>
                <div>
                  <label className="text-sm font-semibold text-foreground mb-1.5 block">Service Needed</label>
                  <select required value={form.service} onChange={update('service')} className="w-full h-12 min-h-[52px] px-4 border border-border rounded-md bg-background text-foreground text-base focus:outline-none focus:ring-2 focus:ring-accent transition">
                    <option value="">Select a service</option>
                    <option>Bed Bugs</option><option>Cockroaches</option><option>Ants</option><option>Rodents</option><option>Wasps</option><option>Other</option>
                  </select>
                </div>
                <div>
                ...
                <button type="submit" className="w-full bg-accent text-accent-foreground font-bold text-lg h-14 rounded-md hover:brightness-110 transition-all active:scale-[0.97] shadow-lg shadow-accent/20">
                  Send My Free Quote Request
                </button>
                <p className="text-center text-muted-foreground text-xs">🔒 Your info is 100% private.</p>
              </div>
            </form>
          </ScrollReveal>
        </div>
